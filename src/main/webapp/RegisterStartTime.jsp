<%@ page import="model.entity.User,model.entity.StartTime,java.time.LocalDate,java.time.format.DateTimeFormatter" %>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%
    User loginUser = (User) session.getAttribute("loginUser");
    StartTime startTime = (StartTime) session.getAttribute("startTime");
    int nowHour = startTime.getStartHour();
    int nowMinut = startTime.getStartMinut();
    String loginUserName = loginUser.getName();
    DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy年MM月dd日");
    String today = LocalDate.now().format(dtf);
    boolean registerSTFlag = true;
    if(request.getAttribute("registerSTFlag")!=null){
    	registerSTFlag = (boolean)request.getAttribute("registerSTFlag");
    }
    %>
<!DOCTYPE html>
<html>
...
<br>
業務開始時刻：
<form method="POST" action="RegisterStartTime">
...
<%= registerSTFlag ?"":"勤務開始時刻を登録できませんでした。" %>
<br><br>
